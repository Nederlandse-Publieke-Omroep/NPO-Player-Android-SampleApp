...
import nl.npo.player.sample_app.databinding.ActivityOfflineBinding
...
class OfflineActivity : BaseActivity() {
    ...
    private fun onItemClicked(sourceWrapper: SourceWrapper) {
        if (sourceWrapper.npoOfflineContent != null) {
            when (sourceWrapper.npoOfflineContent.downloadState.value) {
                NPODownloadState.Finished -> {
                    val offlineSource = sourceWrapper.npoOfflineContent.getOfflineSource()
                    startActivity(
                ...
                is NPODownloadState.Failed, is NPODownloadState.Paused -> {
                    sourceWrapper.npoOfflineContent.startOrResumeDownload()
                }
                is NPODownloadState.InProgress -> {
                    sourceWrapper.npoOfflineContent.pause()
                }
