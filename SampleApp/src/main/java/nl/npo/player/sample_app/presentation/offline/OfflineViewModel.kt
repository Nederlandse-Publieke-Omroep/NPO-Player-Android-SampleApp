...
class OfflineViewModel @Inject constructor(
    ...
    fun createOfflineContent(sourceWrapper: SourceWrapper, errorCallback: (Throwable) -> Unit) {
            ...
            if (mutableOfflineLinkList.value?.indexOfFirst { it.uniqueId == sourceWrapper.uniqueId } != -1) {
                ...
                mutableOfflineLinkList.value =
                    mutableOfflineLinkList.value?.toMutableList()?.apply {
                        val newSource = sourceWrapper.copy(
    ...
    fun deleteOfflineContent(sourceWrapper: SourceWrapper) {
        ...
        viewModelScope.launch {
            ...
            mutableOfflineLinkList.value = mutableOfflineLinkList.value?.toMutableList()?.apply {
    ...
    private fun getStreamLinkListItems() = viewModelScope.launch {
        mutableStreamLinkList.value = streamLinkRepository.getSourceList()
        updateMergedLinkList()
    }

    private fun getUrlLinkListItems() = viewModelScope.launch {
        mutableURLLinkList.value = urlLinkRepository.getSourceList()
        updateMergedLinkList()
    }

    private fun getOfflineLinkListItems() = viewModelScope.launch {
    ...
    private fun updateMergedLinkList() {
        val unionList =
            (urlLinkList.value ?: emptyList()).union(streamLinkList.value ?: emptyList())
